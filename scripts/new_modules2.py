...
def get_timescales(correlation_file,OP,smallest_corr_time, biggest_corr_time, N_exp_to_fit,analyze):
    ...
    return Ctimes, Coeffs, Teff, tau_eff_area, org_corrF, times_out
...
def get_timescales_for_system(folder_path,OP,smallest_corr_time, biggest_corr_time, N_exp_to_fit,analyze,output_path,output_prefix,save_yaml=False,save_txt=True):
    ...
    if new_ana:
        ...
        for i in range(1,len(timescales)):
            if timescales[i][-1]>0:
                if len(residues)>0:
                    print(f'Artificially slow timescale detected for id {i-1}, residue {residues[i-1]}')
                else:
        ...
        if save_yaml:
            ...
            return timescales, eff_times, eff_times_area, residues, org_data, output_prefix
