...
def CorrelationFunctionsLipids(folder_path,begin,end,RM_avail,moleculeType,output_path,systems,CH_bonds,grofile=None,xtcfile=None,tprfile=None):
    """
    ...
    if RM_avail:            
        ...
        if os.path.isfile(output_path+main_name+"/README_correl.yaml"):
            ...
            if 'ANALYSIS' in readme:  
                if moleculeType in readme['ANALYSIS']['CORRELATION_FUNCTIONS']:            
    ...
    if run_again:
        for atom1, atom2 in CH_bonds:
            CalculateCorrelationFunctions(folder_path,begin,end,RM_avail,atom1,atom2,moleculeType,output_path,grofile=None,xtcfile=None,tprfile=None)
            print(f'       {atom1} - {atom2}')
...
class GetRelaxationData():
    ...
    def calc_relax_time(self):
        ...
        Ctimes = 10 ** np.arange(self.smallest_corr_time, self.biggest_corr_time, step_exp)  # units: ps
